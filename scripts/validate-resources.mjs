...
for (const [index, item] of resources.entries()) {
  ...
  if (domainNamePair.has(pairKey)) {
    errors.push(`${row}: duplicate domain+name combination detected (${pairKey})`);
  }
  domainNamePair.add(pairKey);
}

if (resources.length < 60) {
  errors.push(`dataset must contain at least 60 resources; found ${resources.length}`);
}
...
for (const [category, count] of categoryCounts.entries()) {
  if (count < 3) {
    errors.push(`category '${category}' must contain at least 3 active resources; found ${count}`);
  }
}
