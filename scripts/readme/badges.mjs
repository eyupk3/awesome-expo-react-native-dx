...
function colorForSource(sourceKind) {
  if (sourceKind === "official") return "16a34a";
  if (sourceKind === "commercial") return "a855f7";
  return "0284c7";
}
...
export function renderSignalBadges(item) {
  return [
    ...
    badge("source", item.source_kind, colorForSource(item.source_kind)),
